...
const Step1: React.FC<any> = ({signup, setSignup, setStep}) => {
    ...
    const response = (res:any) => {
        if(res?.status == 200) {
            setSignup({...signup, user_id: res?.user_id});
            ...
            setServerError({error: true, message: res?.message});
            setLoading(false);
        }
    }
    ...
    const buttonSubmit = () => {
        ...
        setError({
            ...error,
            first_name: signup?.first_name== "" ?  "Required" : "",
            last_name: signup?.last_name== "" ?  "Required" : "",
            area: (signup?.area== "" || typeof signup?.area == "undefined") ?  "Required" : "",
            mobile: signup?.mobile== "" ?  "Required" : "",
            email: signup?.email== "" ?  "Required" : !ValidateEmail(signup?.email) ? "invalid email" : "",
            password: signup?.password== "" ?  "Required" : !PasswordValidator(signup?.password) ? "didn't match criteria": "",
        });

        if(
            (signup?.area== "" || typeof signup?.area == "undefined") || 
            signup?.first_name== "" || 
            signup?.last_name== "" || 
            signup?.mobile== "" ||
            (signup?.email== "" || !ValidateEmail(signup?.email)) ||
            (signup?.password== "" || !PasswordValidator(signup?.password))
        ) {
            setTimeout(()=>{
                setLoading(false);
            }, 200);
        } else {
            dispatch(initialRegister(signup, response));
        }
            ...
            <>
                <div className="join-card">
                    ...
                    <div className="inner-wrapper">
                        ...
                        <p className="sub_title margin-top-0 margin-bottom-20">Hi! Thank you for joining us! We’re more than happy to welcome you!</p>
                        <div className="form-wrapper">
                            {
                                serverError?.error &&
                                <p className="serverError">{serverError?.message}</p>
                            }
                            ...
                            <div className="row telephone">
                                {/* <IonItem lines="none" className="form">
                                    <IonLabel position="floating">Area code</IonLabel>
                                    <IonInput placeholder="Area Code"></IonInput>
                                </IonItem> */}
                                <IonItem lines="none" className="form">
                                    <div className="required top-5">{error?.area}</div>
                                    <select value={signup?.area} onChange={ (e) => {setSignup({ ...signup, area: e.target.value, number: MobileCombiner(e.target.value, MobileCleaner(signup?.mobile)) }) } } defaultValue={""}>
                                        <option value="" disabled>Area code</option>
